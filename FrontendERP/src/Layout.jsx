...
import { LayoutDashboard, Boxes, Settings, LogOut, Wallet, StickyNoteCheck, Users } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row gap-1 pl-3">
        ...
        <NavItem icon={<Settings size={18} strokeWidth={1.5} />} label="Movimentações" isExpanded={isExpanded} to="/Movimentacoes" />
      </div>

      <div className="ml-2 pl-3 pr-1 border-l border-paper-white/10">
        <NavItem icon={<LogOut size={18} strokeWidth={1.5} />} label="Logout" isExpanded={isExpanded} to="/" />
      </div>
    </nav>
  );
}
  ...
  return (
    <div className="min-h-screen bg-paper-white text-carbon relative">
      ...
      <div className="fixed top-8 right-12 z-50 flex items-center gap-3 bg-paper-white/90 backdrop-blur-xl border border-carbon/10 shadow-sm rounded-[75px] py-1.5 pr-5 pl-1.5 hover:shadow-md hover:border-carbon/20 transition-all duration-500 cursor-pointer group">
        <div className="w-8 h-8 rounded-full bg-carbon text-paper-white flex items-center justify-center text-[12px] font-[400] group-hover:scale-105 transition-transform duration-500">
          M
        </div>
        <div className="flex flex-col justify-center">
          <span className="text-[9px] uppercase tracking-widest text-ash leading-none mb-1">Logado</span>
          <span className="text-[13px] text-carbon font-[400] leading-none">moha</span>
        </div>
      </div>
