...
export default function AddCargo() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await cargosService.createCargo(token, formData);
      navigate('/cargos');
    } catch (error) {
      console.error("Erro ao criar cargo:", error);
      alert("Erro ao criar cargo. Verifique se os dados estão corretos.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <AnimatedPage>
      <div className="min-h-screen bg-[#fafafa] pt-24 pb-12 px-8 text-gray-800 font-sans">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-light text-gray-900 tracking-tight">Novo Cargo</h1>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 flex flex-col gap-6">
            ...
            <div className="mt-6 flex justify-end border-t border-gray-100 pt-6">
              ...
              >
                <Save size={16} /> {loading ? 'Salvando...' : 'Salvar Cargo'}
              </button>
            </div>
          </form>
        </div>
