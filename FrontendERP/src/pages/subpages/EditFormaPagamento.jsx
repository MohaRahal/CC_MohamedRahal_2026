...
export default function EditFormaPagamento() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (id) {
      formasPagamentoService.getFormaPagamentoById(id)
        .then(data => {
          setFormData({
        ...
        .catch(error => {
          ...
          alert("Não foi possível carregar os dados desta forma de pagamento.");
        })
  ...
  try {
    ...
    await formasPagamentoService.updateFormaPagamento(
      token,
      id,
      {
        ...formData,
        codFormaPagamento: Number(id),
    ...
    <AnimatedPage>
      <div className="min-h-screen bg-[#fafafa] pt-24 pb-12 px-8 text-gray-800 font-sans">
        <div className="max-w-2xl mx-auto">
          ...
          <Link to="/formas-pagamento" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-black transition-colors mb-6">
            <ArrowLeft size={16} /> Voltar para Formas de Pagamento
          </Link>

          <div className="mb-8">
            <h1 className="text-3xl font-light text-gray-900 tracking-tight">Editar Forma de Pagamento</h1>
            <p className="text-sm text-gray-500 mt-1">Atualize os dados desta forma de pagamento.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 flex flex-col gap-6">
            <div className="grid grid-cols-1 gap-6">
              <div className="flex flex-col gap-2">
                ...
                <input
                  name="formaPagamento" required value={formData.formaPagamento} onChange={handleChange}
