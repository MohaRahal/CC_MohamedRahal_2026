...
export default function AddProduto() {
  ...
  const handleCreateMarca = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const novaMarca = await marcasService.createMarca(token, { marca: marcaName });
  ...
  const handleCreateGrupo = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const novoGrupo = await gruposService.createGrupo(token, { grupo: grupoName });
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 flex flex-col gap-6">
            ...
            <div className="mt-6 flex justify-end border-t border-gray-100 pt-6">
              <button 
                ...
                className="cursor-pointer flex items-center gap-2 bg-black text-white px-6 py-2.5 rounded-full text-sm font-medium hover:scale-105 hover:bg-gray-800 transition-all shadow-md disabled:opacity-50"
              >
