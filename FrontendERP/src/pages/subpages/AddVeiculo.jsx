...
export default function AddVeiculo() {
  ...
  const fetchDependencias = async () => {
    try {
      ...
      const [marcasData, modelosData, transpData, estData] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await veiculosService.createVeiculo(token, payload);
      navigate('/veiculos');
    } catch (error) {
      console.error("Erro ao criar veículo:", error);
      alert("Erro ao criar veículo. Verifique se os dados estão corretos.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <AnimatedPage>
      <div className="min-h-screen bg-[#fafafa] pt-24 pb-12 px-8 text-gray-800 font-sans">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-light text-gray-900 tracking-tight">Novo Veículo</h1>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 flex flex-col gap-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  ...
                  {transportadores.map(t => (
                    <option key={t.codTransp} value={t.codTransp}>{t.transportador}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  ...
                  {estados.map(e => (
                    <option key={e.codEstado} value={e.codEstado}>{e.estado}</option>
                  ))}
                </select>
              </div>
            ...
            <div className="mt-6 flex justify-end border-t border-gray-100 pt-6">
              ...
              >
                <Save size={16} /> {loading ? 'Salvando...' : 'Salvar Veículo'}
              </button>
            </div>
          </form>
        </div>
