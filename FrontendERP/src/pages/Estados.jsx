...
export default function Estados() {
  ...
  const handleDeleteClick = async (id, nome) => {
    const confirmou = window.confirm(`Tem certeza que deseja excluir o estado "${nome}"?`);
    if (confirmou) {
      try {
        setDeletingId(id);
        const token = localStorage.getItem('token');
        await estadosService.deleteEstado(token, id);
        setEstados(estados.filter(e => e.codEstado !== id));
      } catch (error) {
      ...
      <div className="min-h-screen bg-[#fafafa] pt-24 pb-12 px-8 text-gray-800 font-sans">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex justify-between items-end mb-8">
            <div>
              <h1 className="text-3xl font-light text-gray-900 tracking-tight">Estados</h1>
              <p className="text-sm text-gray-500 mt-1">Gerencie os estados cadastrados no sistema</p>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50">
                    ...
                    <th className="py-4 px-6 text-xs font-medium text-gray-500 uppercase tracking-wider">UF</th>
                  ...
                  {filteredEstados.length === 0 ? (
                    ...
                    filteredEstados.map((estado, idx) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6 text-[13px] text-gray-600">
                          {estado.pais ? estado.pais.pais : '-'}
                        </td>
                        <td className="py-4 px-6 text-[13px] text-gray-600">
                          {formatDate(estado.criado_em)}
                        </td>
                        <td className="py-4 px-6 text-[13px] text-gray-600">
                          {formatDate(estado.atualizado_em)}
                        </td>
                        <td className="py-4 px-6 text-[13px] text-right">
                          <div className="flex items-center justify-end gap-3 transition-opacity">
                            <button 
                              onClick={() => navigate(`/estados/editar/${estado.codEstado}`)}
                              className="text-gray-400 hover:text-blue-600 transition-colors cursor-pointer" title="Editar">
                              <Edit2 size={16} />
                            </button>
                            <button 
                              onClick={() => handleDeleteClick(estado.codEstado, estado.estado)}
                              disabled={deletingId === estado.codEstado}
                              className={`transition-colors cursor-pointer ${deletingId === estado.codEstado ? 'text-gray-300' : 'text-gray-400 hover:text-red-600'}`} 
                              title="Excluir">
                              {deletingId === estado.codEstado ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}
                            </button>
                          </div>
                        </td>
                      </motion.tr>
